...
when "ubuntu","debian","rhel","fedora"
    ...
    template "chef_safe_cron" do
        path node['chef_client']['safe_cron_script']
        source "chef-safe-cron.sh.erb"
        owner "root"
        group "root"
        mode "0755"
        variables(:sleep_time => sleep_time, :env => env, :client_bin => client_bin, :log_file => log_file )
    end
    ...
    if node['chef_client']['cron']['use_cron_d']
      ...
      cron_d "chef-client" do
        ...
        user    "root"
        command node['chef_client']['safe_cron_script']
      end
    else
      cron_d "chef-client" do
        action :delete
